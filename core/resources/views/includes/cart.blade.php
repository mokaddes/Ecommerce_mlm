            ...
            <table class="table table-bordered">
              ...
              <thead>
                <tr>
                  ...
                  <th class="text-center">{{__('SubPoints')}}</th>
              ...
              <tbody id="cart_view_load" data-target="{{route('cart.get.load')}}">
                ...
                <tr>
                    ...
                    <td class="tp">
                        {{ $ttp = $item['points'] * $item['qty'] }}

                    </td>
                    <td class="text-center text-lg">{{PriceHelper::setCurrencyPrice($stp = $item['main_price'] * $item['qty'])}}</td>
  ...
  <div class="card mt-4">
      <div class="card-body">
        <div class="shopping-cart-footer">
            ...
            <div class="text-right column text-lg"><span class="text-muted">{{__('Total Points')}}: </span><span class="text-gray-dark">{{$totalPoints}}</span></div>

            <div class="text-right column text-lg"><span class="text-muted">{{__('Subtotal')}}: </span><span class="text-gray-dark">{{PriceHelper::setCurrencyPrice($totalPrice  - (Session::has('coupon') ? round(Session::get('coupon')['discount'],2) : 0))}}</span></div>
