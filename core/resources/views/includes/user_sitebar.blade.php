...
<div class="col-lg-4">
    <aside class="user-info-wrapper">
      <div class="user-info">
        ...
        <div class="user-data">
          <h4 class="h5">{{$user->first_name}} {{$user->last_name}}</h4><span>{{__('Joined')}} @if (isset($user->created_at)){{$user->created_at->format('M D Y')}} @endif</span>
        </div>
      </div>
      <nav class="list-group">
        ...
        <a class="list-group-item {{ request()->is('user/profile') ? 'active' : '' }}" href="{{route('user.profile')}}"><i class="icon-user"></i>{{__('Profile')}}</a>
            ...
            <a class="list-group-item {{ request()->is('user/referal_manage') ? 'active' : '' }}" href="{{route('user.referal_manage')}}"><i class="icon-users"></i>{{__('Referal Genealogy')}}</a>
