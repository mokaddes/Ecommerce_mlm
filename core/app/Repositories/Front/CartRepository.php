...
class CartRepository
{
    ...
    public function store($request)
    {
        ...
        $item = Item::where('id',$input['item_id'])->select('id','name','photo','discount_price','previous_price','slug','item_type','license_name','license_key', 'points')->first();
        ...
        $option_points = array_sum($input['option_points']);
        ...
        $attribute['option_points'] = $input['option_points'];
        $cart = Session::get('cart');
        ...
        if (!$cart || !isset($cart[$item->id.'-'.$cart_item_key])) {
            ...
            $cart [$item->id.'-'.$cart_item_key] = [
                    ...
                    'attribute_points' => $option_points,
                    "name" => $item->name,
                    "slug" => $item->slug,
                    "qty" => $qty,
